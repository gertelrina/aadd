...
class CIFAR10OneHot(Dataset):
    """
    Класс-обертка для датасета CIFAR-10, который преобразует метки в one-hot формат.
    """
    def __init__(self, root, train=True, transform=None, download=True):
        self.dataset = CIFAR10(root=root, train=train, transform=transform, download=download)
        self.num_classes = 10  # CIFAR-10 содержит 10 классов
...
# Задаем трансформации для предобработки изображений
transform = transforms.Compose([
    transforms.ToTensor(),
    # transforms.Normalize(mean=[0.4914, 0.4822, 0.4465], std=[0.247, 0.243, 0.261])
])

# Загружаем CIFAR-10 с one-hot метками
batch_size = 32
train_dataset = CIFAR10OneHot(root="./data", train=True, transform=transform, download=True)
val_dataset = CIFAR10OneHot(root="./data", train=False, transform=transform, download=True)
...
for epoch in range(epochs):
    ...
    for images, labels in train_tqdm:
        ...
        #     print(f'Error in {preds[k]}, {labels[k]}')
    ...
    report = classification_report(all_labels, all_preds, output_dict=True, zero_division=0, target_names = ["AddGaussianNoise", "PitchShift" , "TimeStretch", "Shift", "ClippingDistortion", "LowPassFilter" , "HighPassFilter","8","9","10" ])
    print(report)
